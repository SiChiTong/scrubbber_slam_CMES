...
class MultiResolutionMap {
   ...
   public:
    ...
    int AddPoints(CloudPtr points, float z_ground);

    float Logp(CloudPtr points, float z_groud, int layer);

    Eigen::MatrixXf Logps(CloudPtr points, float z_groud, int linear_search, int layer);

    inline Eigen::MatrixXf LogpsLow(CloudPtr points, float z_ground, int linear_search) {
