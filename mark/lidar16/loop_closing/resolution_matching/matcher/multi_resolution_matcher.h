...
class MultiResolutionMatcher {
   public:
     ...
     * @return
     */
    float Score(CloudPtr raw_points, float z_ground, SE3& init_pose, float min_score,
                SE3& pose_estimate);

    inline int AddPoints(CloudPtr points, float z_ground) {
        return maps_ptr_->AddPoints(points, z_ground);
    }

   private:
    ScoreBoard::BestOffsetPtr ScoreOneLayer(CloudPtr points, float z_ground, const SE3& init_pose,
                                            int layer, float min_score);

    CloudPtr TrimRawPoints(CloudPtr raw_points, float resolution);
