...
int MultiResolutionMap::AddPoints(CloudPtr points, float z_ground) {
...
float MultiResolutionMap::Logp(CloudPtr points, float z_ground, int layer) {
    int size = points->size();
    Eigen::Vector4f point;
    float pro = 0.0;
    for (int i = 0; i < size; ++i) {
...
Eigen::MatrixXf MultiResolutionMap::Logps(CloudPtr points, float z_ground, int linear_search,
                                          int layer) {
