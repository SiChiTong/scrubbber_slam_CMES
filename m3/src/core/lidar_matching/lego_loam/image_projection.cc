...
//ImageProjection::ImageProjection(const ros::NodeHandle& nh) : nh_(nh) {
...
void ImageProjection::groundRemoval() {
    ...
    //    if (pubGroundCloud.getNumSubscribers() != 0)
    {
        for (size_t i = 0; i <= groundScanInd; ++i) {
            for (size_t j = 0; j < Horizon_SCAN; ++j) {
                if (groundMat.at<int8_t>(i, j) == 1) groundCloud->push_back(fullCloud->points[j + i * Horizon_SCAN]);
            }
        }
    }
