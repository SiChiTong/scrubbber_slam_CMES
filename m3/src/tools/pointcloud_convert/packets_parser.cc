...
int PacketsParser::Setup(const VelodyneConfig &conf) {
    ...
    for (auto &rings_pointcloud : organized_raw_pointcloud_)
        rings_pointcloud = boost::make_shared<PointCloudXYZIT>();
...
void PacketsParser::PaddingPointCloud(const PacketsMsgPtr &scan_msg,
                                      PointCloudXYZIT::Ptr &out_pc_msg_ptr) {
    ...
    int one_ring_points_size =
            packets_size * kScanPerBlock / organized_raw_pointcloud_.size();
    ...
    for (size_t i = 0; i < packets_size; ++i) {
        Unpack(scan_msg->packets[i], organized_raw_pointcloud_);
    }
...
void PacketsParser::Unpack(const velodyne_msgs::VelodynePacket &pkt,
                           std::vector<PointCloudXYZIT::Ptr> &rings_pointcloud) {
    ...
    for (int block = 0; block < kBlocksPerPacket; block++) {
        ...
        for (int firing = 0, k = 0; firing < kVLP16FiringsPerBlock; ++firing) {
            for (int dsr = 0; dsr < kVLP16ScanPerFiring;
                 ++dsr, k += kRawScanSize) {
                ...
                azimuth_corrected_f =
                        azimuth + (*azimuth_corrected_table_)[firing][dsr];
                ...
                double timestamp =
                        pkt_time +
                        (*inner_time_)[block][firing * kVLP16ScanPerFiring + dsr] * 1e-6;

                point.timestamp = timestamp;

                point.ring = rings_map_[dsr];
                ...
                if (raw_distance.uint == 0 ||
                    !isScanValid(azimuth_corrected, distance)) {
                    FilledNAN(point);
                } else {
                ...
                rings_pointcloud[point.ring]->points.emplace_back(point);
                ++rings_pointcloud[point.ring]->width;
            }
        }
    }
}

void PacketsParser::ArrangePointcloud(
        const std::vector<PointCloudXYZIT::Ptr> &rings_pointcloud,
        PointCloudXYZIT::Ptr &out_pc_ptr) {
    int width = 0;
    int height = 0;
    for (size_t i = 0; i < rings_pointcloud.size(); ++i) {
        ...
        if (config_.is_organized) {
            ...
            width =
                    std::max(static_cast<int>(rings_pointcloud[i]->points.size()), width);
            height += 1;
        } else {
...
inline void PacketsParser::FilledNAN(PointXYZIT &point) {
    point.x = HIT_NAN;
    point.y = HIT_NAN;
    point.z = HIT_NAN;
    point.intensity = 0;
...
inline void PacketsParser::FilledFree(PointXYZIT &point) {
    point.x = HIT_FREE;
    point.y = HIT_FREE;
    point.z = HIT_FREE;
    point.intensity = 0;
...
inline bool PacketsParser::isPointValid(const PointXYZIT &point) {
    if (point.x == HIT_NAN || point.x == HIT_FREE ||
        point.y == HIT_NAN || point.y == HIT_FREE ||
        point.z == HIT_NAN || point.z == HIT_FREE)
        return false;
...
inline bool PacketsParser::isScanValid(const PointXYZIT &point) {
    if (point.x < config_.car_front && point.x > config_.car_back &&
        point.y < config_.car_left && point.y > config_.car_right &&
...
void PacketsParser::ComputeCoords(const float distance, const int vert_line_index,
    ...
    if (config_.enable_coordinate_transformation) {
        ...
        point.x = transform_x_of_x * x_coord + transform_y_of_x * y_coord +
                  transform_z_of_x * z_coord;
        point.y = transform_x_of_y * x_coord + transform_y_of_y * y_coord +
                  transform_z_of_y * z_coord;
        point.z = transform_x_of_z * x_coord + transform_y_of_z * y_coord +
                  transform_z_of_z * z_coord;
        point.angle = point.angle + config_.yaw;
        point.angle = point.angle >= 0.0 ? point.angle : point.angle + 360.0;
        point.angle = point.angle <= 360.0 ? point.angle : point.angle - 360.0;
