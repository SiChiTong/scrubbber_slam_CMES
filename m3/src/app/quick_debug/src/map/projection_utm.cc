...
double ProjectionUTM::pi = M_PI;

/* Ellipsoid model constants (actual values here are for WGS84) */
double ProjectionUTM::sm_a = 6378137.0;
double ProjectionUTM::sm_b = 6356752.314;
 ...
 *
 */
double ProjectionUTM::ArcLengthOfMeridian(double phi) {
 ...
 *
 */
double ProjectionUTM::UTMCentralMeridian(int zone) {
    double temp = -183.0 + (zone * 6.0);
 ...
 *
 */
double ProjectionUTM::FootpointLatitude(double y) {
    ...
    alpha_ =
        ((sm_a + sm_b) / 2.0) * (1 + (pow(n, 2.0) / 4) + (pow(n, 4.0) / 64));
 ...
 * Transverse Mercator ProjectionUTM.  Note that Transverse Mercator is not
 ...
 *
 */
void ProjectionUTM::MapLatLonToXY(double phi, double lambda, double lambda0,
                                  UTMPoint &xy) {
 ...
 * MapXYToLatLon
 *
 * Converts x and y coordinates in the Transverse Mercator ProjectionUTM to
 ...
 *
 */
void ProjectionUTM::MapXYToLatLon(double x, double y, double lambda0,
                                  LatLon &philambda) {
    ...
    /* Calculate longitude */
    philambda.lon = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) +
 ...
 *
 */
void ProjectionUTM::LatLonToCartesian(double lat, double lon, UTMPoint &xy) {
 ...
 * ProjectionUTM to a latitude/longitude pair.
 *
 * Inputs:
 *	x - The easting of the point, in meters.
 *	y - The northing of the point, in meters.
 ...
 *
 */
void ProjectionUTM::CartesianToLatLon(double x, double y, int zone,
                                      bool southhemi, LatLon &latlon) {
    double cmeridian;
    ...
    latlon.lat = (latlon.lat / pi * 180.0);
    latlon.lon = (latlon.lon / pi * 180.0);
}
